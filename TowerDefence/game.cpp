...
static void unbindTextures(GLuint shader) {
    ...
    static const char* flags[] = {
        ...
        "hasMetallic","hasRoughness","hasAO"
    };
    for (auto f : flags) {
...
void Game::init() {
    ...
    std::vector<VertexData> gv = {
		// Ground plane vertices with position, normal, color, and texture coordinates
        {-S,Y,-S, 0,1,0, 1,1,1, 0,0},
        { S,Y,-S, 0,1,0, 1,1,1, T,0},
        { S,Y, S, 0,1,0, 1,1,1, T,T},
        {-S,Y,-S, 0,1,0, 1,1,1, 0,0},
        { S,Y, S, 0,1,0, 1,1,1, T,T},
        {-S,Y, S, 0,1,0, 1,1,1, 0,T},
    };
    ...
    for (const auto& s : shp) {
        ...
        for (size_t f = 0; f < s.mesh.num_face_vertices.size(); f++) {
            ...
            for (size_t v = 0; v < fv; v++) {
				...
				// Extract Normal (if available), otherwise use a default normal pointing up.
                ...
                else { vd.nx = 0.0f; vd.ny = 1.0f; vd.nz = 0.0f; }

				// Extract Texture Coordinates (if available), otherwise use default coordinates.
                if (idx.texcoord_index >= 0) {
                    vd.u = attr.texcoords[2 * idx.texcoord_index + 0];
                    vd.v = attr.texcoords[2 * idx.texcoord_index + 1];
                }

				// Extract Diffuse Color from Material (if available), otherwise use a default color.
                ...
                else { vd.r = 0.0f; vd.g = 0.0f; vd.b = 0.0f; }

                buffer.push_back(vd);
            }
			off += fv; // Move to the next face's vertices
        }
    }
}
